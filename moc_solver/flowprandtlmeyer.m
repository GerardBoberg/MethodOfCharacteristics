...
%   $Revision: 1.1.6.6 $  $Date: 2010/09/28 03:14:09 $
...
if ~isnumeric(gamma)
    error('aero:flowprandtlmeyer:notNumeric',...
        'Specific heat ratio inputs must be numeric.');
end
...
if ~isreal(gamma)
    error('aero:flowprandtlmeyer:imaginary',...
        'Specific heat ratio inputs must be real numbers greater than 1.');
end
...
if ~isreal(varargin{1})
    error('aero:flowprandtlmeyer:imaginary',...
        'Mach number inputs must be non-negative real numbers.');
end
...
switch lower(param)
    ...
    case 'nu'
        ...
        if varargin{1}<0
            error('aero:flowprandtlmeyer:prandtlMeyerAngle',...
                'Prandtl-Meyer angle inputs (in degrees) must be real numbers 0 <= NU <= %.17f for GAMMA = %.15f.', ...
                obLoTurn, obLoGamma)
        end
